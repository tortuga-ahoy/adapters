...
export const testExtension = async (extension, props) => {
  ...
  describe.runIf(extension.type === "source")(
    "working correctly as a source extension",
    async () => {
      ...
      await page.goto(extension.source, { waitUntil: "networkidle0" });
      const challenge = await page.$("iframe[src*='cloudflare']");
      if (challenge) {
        await page.close();
